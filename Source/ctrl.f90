...
USE GLOBAL_CONSTANTS, ONLY : UPDATE_DEVICES_AGAIN
...
USE OUTPUT_CLOCKS, ONLY: RSRT_CLOCK
REAL(EB), INTENT(IN) :: T,DT
...
CONTROL_SELECT: SELECT CASE (CF%CONTROL_INDEX)
   ...
   CASE (CUSTOM)
      ...
      RAMP_VALUE = EVALUATE_RAMP(RAMP_INPUT,CF%RAMP_INDEX)
      CF%INSTANT_VALUE = DV%SMOOTHED_VALUE
   ...
   CASE (CF_PERCENTILE)
      ...
      CF%PSUM(0) = 0._EB
      DO NN=1,DV%N_POINTS
         DV2 => DEVICE(CF%INPUT(1)+NN-1)
         CF%PSUM(NN) = CF%PSUM(NN-1) + CF%DL(NN)*DV2%INSTANT_VALUE  ! Partial sums
      ENDDO

      K = 1
      DO NN=1,DV%N_POINTS
         IF (CF%PSUM(NN)>CF%PERCENTILE*CF%PSUM(DV%N_POINTS)) THEN
            K=NN
            EXIT
         ENDIF
      ENDDO
   ...
   CASE (CF_PID)
!	  I_COUNTER,INSTANT_DETEC,AVERAGE_DETEC
      CF%INSTANT_VALUE=0._EB

      SELECT CASE (CF%INPUT_TYPE(1))
         CASE (DEVICE_INPUT)
            DV => DEVICE(CF%INPUT(1))
			IF ((DV%SMOOTHED_VALUE - CF%TARGET_VALUE)>=0) THEN
				CF%STATE_PID=.TURE.
				CF%PREV_STATE_PID = CF%STATE_PID .OR. CF%PREV_STATE_PID
            ENDIF
			IF (CF%PREV_STATE_PID) CF%INSTANT_VALUE = DV%SMOOTHED_VALUE - CF%TARGET_VALUE
			CF%MESH = DV%MESH
         CASE (CONTROL_INPUT)
            IF (.NOT. CONTROL(CF%INPUT(1))%UPDATED) THEN
               CALL EVALUATE_CONTROL(T,CF%INPUT(1),DT,CTRL_STOP_STATUS)
	  ...
	  IF (PID_VALUE >= CF%UPPER_PID) PID_VALUE = CF%UPPER_PID
	  IF (PID_VALUE < CF%LOWER_PID) PID_VALUE = CF%LOWER_PID
   ...
   CASE (CF_ATAN)
      ...
      SELECT CASE (CF%INPUT_TYPE(1))
         CASE (DEVICE_INPUT)
            DV => DEVICE(CF%INPUT(1))
            CF%INSTANT_VALUE = ATAN(DV%SMOOTHED_VALUE)
            CF%MESH = DV%MESH
         CASE (CONTROL_INPUT)
            IF (.NOT. CONTROL(CF%INPUT(1))%UPDATED) THEN
               CALL EVALUATE_CONTROL(T,CF%INPUT(1),DT,CTRL_STOP_STATUS)
               CF => CONTROL(ID)
            ENDIF
            CF%INSTANT_VALUE = ATAN(CONTROL(CF%INPUT(1))%INSTANT_VALUE)
            CF%MESH=CONTROL(CF%INPUT(1))%MESH
         CASE (CONSTANT_INPUT)
            CF%INSTANT_VALUE = ATAN(CF%CONSTANT)
      END SELECT
